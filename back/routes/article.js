...
router.get('/list', (req, res) => {
  // 查詢文章
  const articleQuery = 'SELECT * FROM `Article` WHERE valid = 1'
  connection.execute(articleQuery, (error, articles) => {
    if (error) {
      console.error('Database query error:', error)
      return res.status(500).json({ message: '伺服器錯誤' })
    }
    ...
    connection.execute(likesQuery, (error, likes) => {
      if (error) {
      ...
      const articlesWithLikes = articles.map((article) => {
        const likeData = likes.find((like) => like.target_id === article.id)
        return {
          ...article,
          likes_count: likeData ? likeData.likes_count : 0, // 如果沒有按讚數，設置為 0
        }
      })
...
router.delete('/comments/:id', async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: '評論及其回覆已刪除' })
  } catch (error) {
